...
let antibadword = JSON.parse(
  fs.readFileSync("./lib/database/antibadword.json")
);
let antisticker = JSON.parse(
  fs.readFileSync("./lib/database/antisticker.json")
);
...
let stickerspam = JSON.parse(
  fs.readFileSync("./lib/database/stickerspam.json")
);
...
module.exports = tobz = async (tobz, message) => {
  try {
    ...
    const ownerNumber = "18652809983@c.us";
      ...
      if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
        ...
        if (!check) {
          ...
          tobz
            .reply(
              from,
              `*「 GROUP LINK DETECTOR 」*\nEnviaste un enlace de chat grupal, lo siento, te expulsaron del grupo :(`,
              id
            )
            .then(() => {
    ...
    function isReg(obj) {
      if (obj === true) {
        ...
        return tobz.reply(
          from,
          `No estás registrado como USUARIO de ENRIQUE-BOT \npara registrarte envía ${prefix}login| nombre | edad \n \nexample formato: ${prefix}login | Enrique | 17 \n \n pon primero el nombre xd / nickname`,
          id
    ...
    function cekumur(obj) {
      if (obj === true) {
        ...
        return tobz.reply(
          from,
          `No tienes la edad suficiente para usar Enrique-bot, mínimo 12 años \n \n Puedes volver a registrarte contactando con\ncontactar al propietario : wa.me/18652809983`,
          id
        ); //if user is not registered
      }
    }
    ...
    const kapankah = ["1 Minggu lagi", "1 Bulan lagi", "1 Tahun lagi"];
    ...
    const mess = {
      ...
      magernulissatu: "¡Espere, BOT está escribiendo en el Libro 1!",
      error: {
        St:
          "[❗] Envíe una imagen con el título *#sticker* o una etiqueta de imagen que se haya enviado",
        Ti:
          "[❗] Vuelva a reproducir la etiqueta adhesiva con el comando *#stickertoimg* o la etiqueta adhesiva que se envió",
        Qm: "[❗] Ha ocurrido un error, ¡quizás el tema no esté disponible!",
        Yt3: "[❗] ¡Algo salió mal, no se puede convertir a mp3!",
        Yt4:
          "[❗] Ocurrió un error, tal vez el error fue causado por el sistema.",
        Ig: "[❗] Ocurrió un error, tal vez porque la cuenta es privada",
        Ki: "[❗] ¡El bot no puede eliminar el administrador del grupo!",
        Sp: "[❗] El bot no puede iniciar sesión como administrador",
        Ow: "[❗]El bot no puede emitir propietario",
        Bk: "[❗] Los bots no pueden bloquear al propietario",
        Ad: "[❗] No se puede agregar al contacto",
    ...
    function isStickerMsg(id) {
        ...
        if (i.id === id) {
          if (i.msg >= 12) {
            ...
            tobz
              .reply(
                from,
                `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nSi spameas muchos stickers el bot te echara automaticamente`,
                id
              )
              .then(() => {
            ...
            tobz.reply(
              from,
              `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\n¡Has enviado demasiados stickers! \NNo envíes 5 stickers más o El BOT te expulsara.`,
              id
            );
            return true;
          } else {
      ...
      if (found === false) {
        ...
        fs.writeFileSync(
          "./lib/database/stickerspam.json",
          JSON.stringify(stickerspam)
        );
        return false;
      }
    }
    ...
    function isBadwordMsg(id) {
      ...
      for (let i of msgBadword) {
        if (i.id === id) {
          let msg = i.msg;
          if (msg >= 1) {
            // 1X BADWORD AKAN TERKENA KICK
            kasar === true;
            tobz.reply(
              from,
              `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\n¡Advertencia! ¡Dos veces más palabras duras, Enrique te sacará del grupo!`,
              id
            );
          }
          if (msg >= 3) {
            // 3X BADWORD AKAN TERKENA KICK
            kasar === true;
            tobz
              .reply(
                from,
                `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nHas hablado con dureza en este grupo, ¡Enrique te echara automáticamente!`,
                id
              )
              .then(() => {
                tobz.removeParticipant(groupId, id);
      ...
      if (kasar === false) {
        ...
        msgBadword.push(obj);
        fs.writeFileSync(
          "./lib/database/msgBadword.json",
          JSON.stringify(msgBadword)
        );
        return false;
      }
    }
      ...
      for (let i of msgLimit) {
        if (i.id === id) {
          if (i.msg >= 8) {
            ...
            tobz.reply(
              from,
              `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 」*\nLo sentimos, bloqueamos su cuenta debido a SPAM y no se puede DESBLOQUEAR.`,
              id
            );
            ...
            tobz.reply(
              from,
              `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 」*\n¡Tu número ha sido detectado como spam! \NNo envíes 5 mensajes más o tu número es AUTO BLOQUEO.`,
              id
            );
            return true;
          } else {
      ...
      for (let i of limit) {
        if (i.id === id) {
          ...
          if (limits >= limitCount) {
            ...
            tobz.reply(
              from,
              `Tu comando BOT ha alcanzado el límite, inténtalo mañana :)`,
              id
            );
            return true;
          } else {
    ...
    if (body === "#mute" && isMuted(chatId) == true) {
      if (isGroupMsg) {
        if (!isAdmin)
          return tobz.reply(
            from,
            "Lo sentimos, este comando solo puede ser realizado por el administrador de ENRIQUE",
            id
          );
        ...
        tobz.reply(
          from,
          "¡Bot ha sido silenciado en este chat! #unmute para reactivar!",
          id
        );
      } else {
        if (isMsgLimit(serial)) {
    ...
    if (body === "#unmute" && isMuted(chatId) == false) {
      if (isGroupMsg) {
        ...
        tobz.reply(from, "¡Se ha desactivado el sonido del bot!", id);
      } else {
        if (isMsgLimit(serial)) {
          return;
        } else {
    ...
    if (body === "#unbanchat") {
      if (!isOwner)
        return tobz.reply(
          from,
          "Lo sentimos, este comando solo lo puede realizar el creador del bot.",
          id
        );
      ...
      tobz.reply("¡El chat global ha sido deshabilitado!");
    }
    if ((isMuted(chatId) && banChat() && !isBlocked && !isBanned) || isOwner) {
      switch (command) {
        case prefix + "banchat":
          ...
          tobz.reply("¡Se ha habilitado el chat global!");
          break;
        ...
        case prefix + "ttp":
          ...
          try {
            const string = body.toLowerCase().includes("#ttp")
              ? body.slice(5)
              : body.slice(5);
            if (args) {
              if (quotedMsgObj == null) {
                ...
                if (gasMake.status == true) {
                  try {
                    ...
                    await tobz.reply(from, "Fallo al crear.", id);
                  }
                } else {
              ...
              await tobz.reply(from, "No puede estar vacío.", id);
            }
          } catch (error) {
            console.log(error);
          }
        ...
        case prefix + "ttp2":
          ...
          if (isLimit(serial))
            return tobz.reply(
              from,
              `lo siento ${pushname}, Su límite de cuota se ha agotado, escriba ${prefix}limit para verificar su límite de cuota`,
              id
            );
          if (args.length === 1)
            return tobz.reply(
              from,
              `Enviar comando *#ttp2[Texto]*, por ejemplo *# ttp2 Enrique*`,
              id
            );
          ...
          await tobz.sendStickerfromUrl(
            from,
            `https://api.vhtear.com/textmaker?text=${ttp2t}&warna=${rttp2}&apikey=${vhtearkey}`
          );
          break;
        case prefix + "ttg":
          if (isReg(obj)) return;
          ...
          try {
            if (quotedMsgObj == null) {
              if (args.length === 1)
                return tobz.reply(
                  from,
                  `Enviar comando *#ttg [Texto]*, por ejemplo *#ttg ENRIQUE *`,
                  id
                );
              await tobz.sendStickerfromUrl(
                from,
                `https://api.vhtear.com/textxgif?text=${body.slice(
                  5
                )}&apikey=${vhtearkey}`
              );
              limitAdd(serial);
              ...
              await tobz.sendStickerfromUrl(
                from,
                `https://api.vhtear.com/textxgif?text=${quotedMsgObj}&apikey=${vhtearkey}`
              );
              limitAdd(serial);
            }
          } catch (e) {
            console.log(e);
            tobz.reply(from, "Lo siento, el servidor no funciona");
          }
          break;
        case prefix + "magernulis1": // BY MFARELS
          ...
          if (args.length === 4)
            return await tobz.reply(
              from,
              "Enviar comando *#magernulis1 - [Nombre] - [Clase] - [Texto]* \n \n *Ejemplo:* \n # magernulis1 --ENRIQUE - 7B - Seguirme en instagram",
              id
          ...
          const panjangBaris = panjangKalimat
            .split("\n")
            .slice(0, 30)
            .join("\n");
          const panjangBarisNama = panjangNama
            .split("\n")
            .slice(0, 30)
            .join("\n");
          const panjangBarisKelas = panjangKelas
            .split("\n")
            .slice(0, 30)
            .join("\n");
          var months = [
          ...
          spawn("convert", [
            ...
            .on("error", () =>
              tobz.reply(from, "Error vuelva a intentarlo", id)
            )
            .on("exit", () => {
              tobz.sendImage(from, "instagram:@estudiaos*", id);
            });
          break; // BY MFARELS
        case prefix + "stickertoimg":
          ...
          if (quotedMsg && quotedMsg.type == "sticker") {
            ...
            tobz.reply(from, `[ESPERA] En curso⏳ ¡espera!`, id);
            const imageBase64 = `data:${
              quotedMsg.mimetype
            };base64,${mediaData.toString("base64")}`;
            await tobz.sendFile(
              ...
              imageBase64,
              "imagesticker.jpg",
              "Se ha convertido el sticker en imagen con exito",
              id
            );
          } else if (!quotedMsg)
            return tobz.reply(
              from,
              `Etiqueta el sticker que quieras usar como imagen.`,
              id
            );
          break;
        ...
        case prefix + "sgif": // MRHRTZ
          tobz.reply(from, `[ESPERAR] En curso⏳ ¡espere ± 1 min!`, id);
          if ((isMedia && type === "video") || mimetype === "image/gif") {
            try {
              const mediaData = await decryptMedia(message, uaOverride);
              await tobz.sendMp4AsSticker(from, mediaData, {
                fps: 10,
              ...
              tobz.reply(
                from,
                `¡El tamaño del medio es demasiado grande! por favor reduzca la duración del video.`
              );
            }
          } else if (
            ...
            tobz.reply(
              from,
              `Error ⚠️ Solo puede ser video / gif si el archivo multimedia tiene la forma de una imagen mecanografiada #stickergif`,
              id
            );
          }
          break;
        ...
        case prefix + "slightning":
          ...
          if (isMedia && type === "image") {
            ...
            await tobz.reply(
              from,
              `¡Formato incorrecto! \N⚠️ Envíe imágenes con #stickerlightning`,
              id
            );
          }
          break;
        ...
        case prefix + "sfire":
          ...
          if (isMedia && type === "image") {
            ...
            await tobz.reply(
              from,
              `¡Formato incorrecto! \N⚠️ Envíe imágenes con #stickerfire`,
              id
            );
          }
          break;
        case prefix + "blackpink":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Enviar comando *#blackpink [Texto]*, por ejemplo *#blackpink Enrique *`,
              id
            );
          ...
          if (blpk.length > 10)
            return tobz.reply(
              from,
              "*¡Texto demasiado largo!* N_Máximo 10 letras!_",
              id
            );
          await tobz.sendFileFromUrl(
            from,
            `https://api.vhtear.com/blackpinkicon?text=${blpk}&apikey=${vhtearkey}`,
            "blackpink.jpg",
            "",
            id
          );
        ...
        case prefix + "thunder":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Enviar comando *#thunder [Texto]*, por ejemplo *#thunder Enrique*`,
              id
            );
          tobz.reply(from, mess.wait, id);
        ...
        case prefix + "pornhub":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Enviar comando *#pornhub [| Texto1 | Texto2]*, por ejemplo *#pornhub |Enrique |bot*`,
              id
            );
          ...
          if (argz.length >= 2) {
            ...
            if (lpornhub.length > 10)
              return tobz.reply(
                from,
                "*¡Texto1 demasiado largo!* N_¡Máximo 10 letras! _",
                id
              );
            if (lpornhub2.length > 10)
              return tobz.reply(
                from,
                "*¡Texto2 demasiado largo!* N_¡Máximo 10 letras! _",
                id
              );
            tobz.sendFileFromUrl(
              from,
              `https://api.vhtear.com/pornlogo?text1=${lpornhub}&text2=${lpornhub2}&apikey=${vhtearkey}`
            );
            await limitAdd(serial);
          } else {
            await tobz.reply(
              from,
              `¡Formato incorrecto! \N [❗] Enviar comando *#pornhub [| Texto1 | Texto2]*, ejemplo *#pornhub |Enrique|Bot*`,
              id
            );
          }
          break;
        case prefix + "login": // NAMBAHIN NOMOR DI DATABASE
          argz = body.trim().split("|");
          if (argz.length >= 2) {
            ...
            if (isNaN(umurnye))
              return await tobz.reply(from, "¡La edad debe ser un número!", id);
            if (umurnye >= 40)
              return await tobz.reply(
                from,
                "Eres demasiado mayor, vuelve a tu juventud para usar el BOT",
                id
              );
            ...
            if (obj === true) {
              return tobz.reply(from, "ya estás registrado", id); // BAKAL RESPON JIKA NO UDAH ADA
            } else {
              const mentah = await tobz.checkNumberStatus(nonye); // PENDAFTARAN
              const msg = monospace(`El registro es exitoso con SN: ${SN} en ${moment().format(
                "DD/MM/YY HH:mm:ss"
              )}
₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋
[NOMBRE]: ${jenenge} [@${nonye.replace(/[@c.us]/g, "")}]
[NUMERO]: wa.me/${nonye.replace("@c.us", "")}
[EDAD]: ${umurnye}
⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻
Para utilizar el bot, envíe ${prefix}menu
Total de usuarios registrados ${pendaftar.length}`);
              const hasil = mentah.canReceiveMessage ? msg : false;
            }
          } else {
            await tobz.reply(
              from,
              `El formato que ingresaste es incorrecto, envíalo ${prefix}daftar |Nombre|Edad\n\nformato de muestra: ${prefix}daftar |Enrique|17\n\n pon siempre primero el nombre`,
              id
            ); //if user is not registered
          }
          break;
        case prefix + "login2":
          if (!isAdmin)
            return tobz.reply(
              from,
              "Este comando solo puede ser utilizado por administrador de el BOT",
              id
            );
          ...
          if (umurnya >= 40)
            return await tobz.reply(
              from,
              "La edad es demasiado vieja hermano, máximo 40 yaa: D",
              id
            );
          var found = false;
          ...
          if (found !== false) {
            ...
            const result = monospace(`La actualización de datos se realiza correctamente con SN: ${SN} en ${moment().format(
              "DD/MM/YY HH:mm:ss"
            )}
₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋
[Nombre]: ${updated.nama} | @${updated.id.replace(/[@c.us]/g, "")}
[Numero]: wa.me/${updated.id.replace("@c.us", "")}
[Edad]: ${updated.umur}
⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻
Total de usuarios registrados ${pendaftar.length}`);
            ...
            fs.writeFileSync(
              ...
              JSON.stringify(pendaftar)
            );
            tobz.sendTextWithMentions(from, result, id);
          } else {
            tobz.reply(
              from,
              `${monospace(`La base de datos no tiene ese número, hermano`)}`,
              id
            );
          }
          break;
...
*➸ *Nombre : ${groupname}* 
*➸ Miembros : ${totalMem}*
*➸ Welcome : ${welgrp ? "Activado" : "Desactivado"}*
*➸ Left : ${leftgrp ? "Activado" : "Desactivado"}*
*➸ Anti Sticker : ${stprt ? "Activado" : "Desactivado"}*
*➸ Anti Link : ${antlink ? "Activado" : "Desactivado"}*
*➸ Anti Badword : ${antbad ? "Activado" : "Desactivado"}*
...
${desc}`
        ...
        case prefix + "tts":
          ...
          try {
            if (args.length === 1)
              return tobz.reply(
                from,
                "Enviar comando *#tts [Idioma] [Texto]*, por ejemplo *#tts id hola a todos*"
              );
            var dataBhs = args[1];
            const ttsHZ = require("node-gtts")(dataBhs);
            var dataText = body.slice(8);
            if (dataText === "")
              return tobz.reply(from, "Ingrese el texto", id);
            if (dataText.length > 500)
              return tobz.reply(from, "¡El texto es demasiado largo!", id);
        ...
        case prefix + "moneda":
          ...
          if (side == 1) {
            tobz.sendStickerfromUrl(
              from,
              "https://i.ibb.co/YTWZrZV/2003-indonesia-500-rupiah-copy.png",
              {
                method: "get",
              }
            );
            ...
            tobz.sendStickerfromUrl(
              from,
              "https://i.ibb.co/bLsRM2P/2003-indonesia-500-rupiah-copy-1.png",
              {
                method: "get",
              }
            );
        ...
        case prefix + "owner":
        case prefix + "creator":
          tobz.sendContact(chatId, `18652809983@c.us`);
          tobz.reply(
            from,
            "Ese es el número de mi novia, uh, me refiero a mi dueño",
            id
          );
          break;
        case prefix + "resetsticker":
          ...
          if (!args.length === 1)
            return tobz.reply(
              from,
              `Ingrese el número, *RECUERDA PONER EL PREFIJO DEL PAIS* \ ejemplo: #resetsticker 34633656432 / #resetsticker @member`,
              id
            );
          const nomebr = args[1];
          ...
          if (found !== false) {
            stickerspam[found].msg = 1;
            const result = "DB Sticker Spam se ha restablecido";
            ...
            tobz.reply(
              from,
              `Lo sentimos, ese número no está registrado en la base de datos!`,
              id
            );
          }
          break;
        case prefix + "resetbadword":
          ...
          if (!args.length === 1)
            return tobz.reply(
              from,
              "Ingrese el número, * REECUERDA PONER EL PREFIJO DEL PAIS * \nejemplo: #resetbadword 34644656576 / #resetbadword @member"
            );
          const nomer = args[1];
          ...
          if (found !== false) {
            ...
            const result =
              "DB Se ha restablecido el spam de palabras incorrectas";
            ...
            tobz.reply(
              from,
              `${monospace(`No hay número en la base de datos, hermano.`)}`,
              id
            );
          }
          break;
        ...
        case prefix + "antilink":
          ...
          if (!isGroupAdmins)
            return tobz.reply(
              from,
              `¡Este comando solo puede ser utilizado por LOS ADMINS DEL GRUPO`,
              id
            );
          if (!isBotGroupAdmins)
            return tobz.reply(
              from,
              `¡Este comando solo se puede usar si el bot es un administrador!`,
              id
            );
          if (args[1] == "enable") {
            ...
            tobz.reply(from, `Seleccione enable o disable.`, id);
          }
          break;
        ...
        case prefix + "antibadword":
          ...
          if (args[1] == "enable") {
            ...
            if (cek) {
              return tobz.reply(
                from,
                `*「 ANTI BADWORD 」*\nHa sido activado en este grupo`,
                id
              );
            } else {
              antibadword.push(chatId);
              fs.writeFileSync(
                "./lib/database/antibadword.json",
                JSON.stringify(antibadword)
              );
              tobz.reply(
                from,
                `*「 ANTI BADWORD 」*\nAtención para los miembros del grupo ${name} Hermano\n¡Por favor, no te pongas tóxico aquí o Enrique te echara!`,
                id
              );
            }
          } else if (args[1] == "disable") {
            ...
            if (!cek) {
              return tobz.reply(
                from,
                `*「 ANTI BADWORD 」*\nHa sido inhabilitado en este grupo`,
                id
              );
            } else {
        ...
        case prefix + "nsfw":
          ...
          if (args.length === 1)
            return tobz.reply(from, "Seleccione enable o disable!", id);
          if (args[1].toLowerCase() === "enable") {
            ...
            if (cek) {
              ...
              fs.writeFileSync(
                ...
                JSON.stringify(nsfw_)
              );
              tobz.reply(
                from,
                "¡NSFW se ha activado con éxito en este grupo! enviar comando *#nsfwMenu* para averiguar el menú",
                id
              );
            }
            ...
            var cek = nsfw_.includes(chatId);
            if (cek) {
              return tobz.reply(from, `NSFW desactivado en este grupo`, id);
            } else {
              ...
              tobz.reply(from, "NSFW desactivado con éxito en este grupo!", id);
            }
          } else {
            tobz.reply(from, "Seleccione enable o disable!", id);
          }
        ...
        case prefix + "simi":
          ...
          if (!isAdmin)
            return tobz.reply(
              from,
              "Perintah ini hanya bisa di gunakan oleh Admin Elaina!",
              id
            ); // Hanya Admin yang bisa mengaktifkan
          if (args.length === 1)
            return tobz.reply(from, "Pilih enable atau disable!", id);
          if (args[1].toLowerCase() === "enable") {
            var cek = simi_.includes(chatId);
            if (cek) {
              return tobz.reply(
                from,
                `Simsimi Sudah diaktifkan di grup ini`,
                id
              );
            } else {
              simi_.push(chat.id);
              fs.writeFileSync(
                ...
                JSON.stringify(simi_)
              );
              tobz.reply(
                from,
                "Simsimi berhasil di aktifkan di group ini! Kirim perintah *# [teks]*\nContoh : *# halo*",
                id
              );
            }
          } else if (args[1].toLowerCase() === "disable") {
            ...
            if (cek) {
              ...
              tobz.reply(
                from,
                "Simsimi berhasil di nonaktifkan di group ini!",
                id
              );
            }
          } else {
        ...
        case prefix + "group":
          ...
          if (!isGroupAdmins)
            return tobz.reply(
              from,
              "Esta función solo puede ser utilizada por administradores de grupo",
              id
            );
          if (!isBotGroupAdmins)
            return tobz.reply(
              from,
              "Esta función solo se puede utilizar cuando el bot es un administrador",
              id
            );
          if (args.length === 1)
            return tobz.reply(from, "Seleccione open o close!", id);
          if (args[1].toLowerCase() === "open") {
            tobz.setGroupToAdminsOnly(groupId, false);
            tobz.sendTextWithMentions(
              from,
              `El administrador ha abierto el grupo @${sender.id.replace(
                "@c.us",
                ""
            ...
            tobz.sendTextWithMentions(
              from,
              `El grupo ha sido cerrado por el administrador @${sender.id.replace(
                "@c.us",
                ""
              )}\nAhora *solo los administradores* pueden enviar mensajes`
            );
          } else {
            tobz.reply(from, "Seleccione open o close!", id);
          }
          break;
        case prefix + "left":
          if (isReg(obj)) return;
          ...
          if (args[1].toLowerCase() === "enable") {
            ...
            tobz.reply(
              from,
              "¡La función de #left se ha activado con éxito en este grupo!",
              id
            );
          } else if (args[1].toLowerCase() === "disable") {
            ...
            tobz.reply(
              from,
              "¡La función de #left se ha desactivado con éxito en este grupo!",
              id
            );
          } else {
            tobz.reply(from, "Seleccione enable o disable!", id);
        ...
        case prefix + "welcome":
          ...
          if (args[1].toLowerCase() === "enable") {
            ...
            tobz.reply(
              from,
              "¡La función de bienvenida se ha activado con éxito en este grupo!",
              id
            );
          } else if (args[1].toLowerCase() === "disable") {
            ...
            tobz.reply(
              from,
              "¡La función de bienvenida se ha desactivado con éxito en este grupo!",
              id
            );
          } else {
            tobz.reply(from, "Seleccione enable o disable!", id);
        ...
        case prefix + "otakudesu":
          ...
          if (isLimit(serial))
            return tobz.reply(
              from,
              `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`,
              id
            );
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#otakudesu [consulta]* \nEjemplo: *#otakudesu naruto gay*",
              id
            );
          const animes = await axios.get(
            "https://mhankbarbars.herokuapp.com/api/otakudesu?q=" +
              body.slice(7) +
              "&apiKey=" +
              barbarkey
          );
        ...
        case prefix + "kusonime":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#kusonime [consulta]* \nEjemplo: *#kusonime goku le gana*",
              id
            );
          const animeq = await axios.get(
            "https://tobz-api.herokuapp.com/v1/kuso?q=" + body.slice(7)
          );
        ...
        case prefix + "dewabatch":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#dewabatch [consulta]* \nEjemplo: *#dewabatch goku*",
              id
            );
          const animek = await axios.get(
            "https://tobz-api.herokuapp.com/v1/dewabatch?q=" + body.slice(7)
          );
        ...
        case prefix + "komiku":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#komiku [consulta]* \nEjemplo: *#komiku goku*",
              id
            );
          const animep = await axios.get(
            "https://mhankbarbars.herokuapp.com/api/komiku?q=" +
              body.slice(7) +
              "&apiKey=" +
              barbarkey
          );
        ...
        case prefix + "pinterest":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#pinterest [consulta]* \nEjemplo: *#pinterest Enrique*",
              id
            );
          const ptrsq = body.slice(11);
          const ptrst = await fetch(
            `https://api.vhtear.com/pinterest?query=${ptrsq}&apikey=${vhtearkey}`
          );
          if (!ptrst.ok)
            throw new Error(`Error Pinterest : ${ptrst.statusText}`);
          ...
          await tobz.sendImage(
            from,
            base64,
            "ptrs.jpg",
            `*Pinterest*\n \n *Resultados de la búsqueda : ${ptrsq}*`
          );
          await limitAdd(serial);
          break;
        case prefix + "nhview":
          ...
          if (!isNsfw)
            return tobz.reply(
              from,
              "comando/comando NSFW no activado en este grupo!",
              id
            );
          if (isLimit(serial))
            return tobz.reply(
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#nhview [212121]* \nEjemplo: *#nhview 321421*",
              id
            );
          const nhsh = body.slice(11);
          const nhsh2 = await axios.get(
            "https://mnazria.herokuapp.com/api/nhentai?code=" + nhsh
          );
        ...
        case prefix + "loli":
          ...
          const loli = await axios.get(
            `https://api.vhtear.com/randomloli&apikey=${vhtearkey}`
          );
        ...
        case prefix + "waifu":
          ...
          const waifu = await axios.get(
            "https://tobz-api.herokuapp.com/api/waifu"
          );
          tobz.sendFileFromUrl(
            from,
            waifu.data.image,
            "Waifu.jpg",
            `➸ Nombre : ${waifu.data.name}\n➸ Descripcion : ${waifu.data.desc}\n\n➸ Fuente : ${waifu.data.source}`,
            id
          );
          await limitAdd(serial);
          break;
        ...
        case prefix + "randomnekonime":
          ...
          const nekonime = await axios.get(
            `https://tobz-api.herokuapp.com/api/nekonime`
          );
        ...
        case prefix + "randomtrapnime":
          ...
          const trapnime = await axios.get(
            "https://tobz-api.herokuapp.com/api/nsfwtrap"
          );
        ...
        case prefix + "randomhentai":
          ...
          const hentai = await axios.get(
            `https://tobz-api.herokuapp.com/api/hentai`
          );
        ...
        case prefix + "randomnsfwneko":
          ...
          if (!isNsfw)
            return tobz.reply(
              from,
              "comando/comando NSFW no se ha activado en este grupo!",
              id
            );
          if (isLimit(serial))
            return tobz.reply(
          ...
          const nsfwneko = await axios.get(
            "https://tobz-api.herokuapp.com/api/nsfwneko"
          );
        ...
        case prefix + "randomanime":
          ...
          const ranime = await axios.get(
            "https://tobz-api.herokuapp.com/api/randomanime"
          );
        ...
        case prefix + "randomblowjob":
          ...
          const sblow = await axios.get(
            "https://tobz-api.herokuapp.com/api/nsfwblowjob"
          );
        ...
        case prefix + "randomhug":
          ...
          const shug = await axios.get(
            "https://tobz-api.herokuapp.com/api/hug"
          );
        ...
        case prefix + "randomcry":
          ...
          const scry = await axios.get(
            "https://tobz-api.herokuapp.com/api/cry"
          );
        ...
        case prefix + "randomkiss":
          ...
          const skiss = await axios.get(
            "https://tobz-api.herokuapp.com/api/kiss"
          );
        ...
        case prefix + "subreddit":
          ...
          try {
            const response1 = await axios.get(
              "https://meme-api.herokuapp.com/gimme/" + sr + "/"
            );
            const {
              postLink,
              title,
            ...
            if (nsfw == true) {
              if (isGroupMsg && isNsfw) {
                ...
                await tobz.reply(
                  from,
                  `Nsfw no se ha activado en Grupo *${name}*`,
                  id
                );
              }
            } else {
        ...
        case prefix + "nhder":
          ...
          if (args.length >= 2) {
            ...
            url["short"] = shortener;
            short.push(url);
            const caption = `*DESCARGADOR NEKOPOI*\n\nLink: ${shortener}`;
            tobz.sendText(from, caption);
            limitAdd(serial);
          } else {
            tobz.sendText(
              from,
              "Lo siento por favor ingrese el código nuclear"
            );
          }
          break;
        ...
        case prefix + "quotesnime":
          ...
          const skya = await axios.get(
            "https://tobz-api.herokuapp.com/api/quotesnime/random"
          );
          skya_ = skya.data;
          tobz.reply(
            from,
          ...
          try {
            ...
            if (!parsed) {
              await tobz.sendFileFromUrl(
                ...
                errorurl2,
                "[error.png]",
                "💔️ Lo siento, no se encontró ningún anime",
                id
              );
              return null;
            }
...
✨️ *Nombre:* ${name}
💌️ *Nombres alternativos:* ${alternative_names}
🌐️ *URL*: ${url}`;
        ...
        case prefix + "jadwalsholat":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `[❗] Kirim perintah *#jadwalShalat [ Daerah ]*\ncontoh : *#jadwalShalat Tangerang*\nUntuk list daerah kirim perintah *#listDaerah*`
            );
          const daerah = body.slice(14);
          const jadwalShalat = await axios.get(
            `https://api.vhtear.com/jadwalsholat?query=${daerah}&apiKey=${vhtearkey}`
          );
          if (jadwalShalat.data.error)
            return tobz.reply(from, jadwalShalat.data.error, id);
          const {
            Shubuh,
            Zduhur,
            Ashr,
        ...
        case prefix + "quran":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah Surah Quran kamu dengan cara ketik perintah :\n*#quran* [ Urutan Surat ]\nContoh :\n*#quran 1*`,
              id
            );
          ...
          await tobz
            ...
            .catch((e) =>
              tobz.reply(
                from,
                `*Terdapat kesalahan saat mencari surat ${args[1]}*`,
                id
              )
            );
          await limitAdd(serial);
        ...
        case prefix + "listsurah": // ARUGAZ
          ...
          try {
            axios
              .get(
                "https://raw.githubusercontent.com/ArugaZ/scraper-results/main/islam/surah.json"
              )
              .then((response) => {
                let hehex =
        ...
        case prefix + "infosurah": // ARUGAZ
          ...
          if (args.length == 1)
            return tobz.reply(
              from,
              `Kirim perintah *#infosurah [ Nama Surah ]*\nContoh : *#infosurah al-fatihah*`,
              message.id
            );
        ...
        case prefix + "tafsir": // ARUGAZ
          ...
          if (args.length == 1)
            return tobz.reply(
              from,
              `Kirim perintah *#tafsir [ Nama Surah ] [ Ayat ]*\nContoh : *#tafsir al-fatihah 2*`,
              message.id
            );
          ...
          try {
            ...
            if (!isNaN(nmr)) {
              var responsih = await axios.get(
                "https://api.quran.sutanlab.id/surah/" + nmr + "/" + args[2]
              );
            ...
            tobz.reply(
              from,
              "Data tidak ditemukan, mungkin nama surah/ayat salah",
              id
            );
          }
          break;
        ...
        case prefix + "infogempa":
          ...
          const bmkg = await axios.get(
            "http://tobz-api.herokuapp.com/api/infogempa"
          );
          const {
            potensi,
            koordinat,
        ...
        case prefix + "ssphone":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#ssphone [linkWeb]* \nEjemplo: *#ssphone https://neonime.vip*",
              id
            );
          const ssphone = body.slice(9);
          tobz.sendFileFromUrl(
            from,
            `https://api.vhtear.com/ssweb?link=${ssphone}&type=phone&apikey=${vhtearkey}`,
            "ssphone.jpg",
            "",
            id
          );
        ...
        case prefix + "sspc":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#sspc [linkWeb]* \nEjemplo: *#sspc https://neonime.vip*",
              id
            );
          const sspc = body.slice(6);
          tobz.sendFileFromUrl(
            from,
            `https://api.vhtear.com/ssweb?link=${sspc}&type=pc&apikey=${vhtearkey}`,
            "sspc.jpg",
            "",
            id
          );
        ...
        case prefix + "shorturl":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#shorturl [linkWeb]* \nEjemplo: *#shorturl https://neonime.vip*",
              id
            );
          const sorturl = body.slice(10);
          const surl = await axios.get(
            "https://tobz-api.herokuapp.com/api/shorturl?url=" + sorturl
          );
        ...
        case prefix + "cuaca":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#clima [lugar]* \nEjemplo: *#clima Madrid",
              id
            );
          const tempat = body.slice(7);
          const weather = await axios.get(
            "https://mhankbarbars.herokuapp.com/api/cuaca?q=" +
              tempat +
              "&apiKey=" +
              barbarkey
          );
          ...
          if (weatherr.error) {
            ...
            tobz.reply(
              from,
              `➸ El lugar : ${weatherr.result.tempat}\n\n➸ Viento : ${weatherr.result.angin}\n➸ Clima : ${weatherr.result.cuaca}\n➸ Descripcion : ${weatherr.result.desk}\n➸ Humedad : ${weatherr.result.kelembapan}\n➸ Temperatura : ${weatherr.result.suhu}\n➸ Aire : ${weatherr.result.udara}`,
              id
            );
            limitAdd(serial);
          }
        ...
        case prefix + "spamcall":
          ...
          if ((!isOwner, !isAdmin))
            return tobz.reply(
              from,
              "Este comando es solo para el creador del bot y los admins",
              id
            );
          arg = body.trim().split(" ");
          console.log(...arg[1]);
          ...
          console.log(spam);
          const call2 = await axios.get(
            "https://tobz-api.herokuapp.com/api/spamcall?no=" + spam
          );
        ...
        case prefix + "ytmp4":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Enviar comando *#ytmp4 [Link Yt]*, por ejemplo, envíe el comando *#readme*`
            );
          let isLin = args[1].match(
            /(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/
          );
          if (!isLin) return tobz.reply(from, mess.error.Iv, id);
          try {
            tobz.reply(from, mess.wait, id);
            const ytvh = await fetch(
              `http://api.vhtear.com/ytdl?link=${args[1]}&apikey=${vhtearkey}`
            );
            if (!ytvh.ok)
              throw new Error(`Error Get Video : ${ytvh.statusText}`);
            const ytvh2 = await ytvh.json();
            if (ytvh2.status == false) {
              tobz.reply(
                from,
                `*Lo sentimos Hubo un error al obtener los datos, seleccione otro medio ...*`,
                id
              );
            } else {
              ...
              if (Number(ytvh2.result.size.split(" MB")[0]) > 30.0)
                return tobz.sendFileFromUrl(
                  from,
                  ytvh2.result.UrlVideo,
                  `${title}.mp4`,
                  `*「 YOUTUBE MP4 」*\n\n• *Titulo* : ${ytvh2.result.title}\n• *Tamaño del archivo* : ${ytvh2.result.size}\n\n__Lo sentimos, la duración del video es de más de 30 MB. Descargue el video a través del enlace a continuación_.\n${ytvh2.result.UrlVideo}`,
                  id
                );
              tobz.sendFileFromUrl(
                from,
                ...
                (await tobz
                  .sendFileFromUrl(from, UrlVideo, `${title}.mp4`, "", id)
                  .catch(() => tobz.reply(from, mess.error.Yt4, id)));
              await limitAdd(serial);
            }
          } catch (err) {
        ...
        case prefix + "play":
          ...
          if (isLimit(serial))
            return tobz.reply(
              from,
              `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #ceklimit Untuk Mengecek Kuota Limit Kamu`,
              id
            );
          if (args.length == 1)
            return tobz.reply(
              from,
              `Para buscar una canción de youtube \n \nUso:#play el título de la canción`,
              id
            );
          try {
            tobz.reply(from, mess.wait, id);
            const serplay = body.slice(6);
            const webplay = await fetch(
              `https://api.vhtear.com/ytmp3?query=${serplay}&apikey=${vhtearkey}`
            );
            if (!webplay.ok)
              throw new Error(`Error Get Video : ${webplay.statusText}`);
            const webplay2 = await webplay.json();
            if (webplay2.status == false) {
              tobz.reply(
                from,
                `*Lo sentimos, hubo un error al recuperar datos, elija otro medio ...*`,
                id
              );
            } else {
              if (Number(webplay2.result.size.split(" MB")[0]) >= 10.0)
                return tobz.reply(
                  from,
                  "Lo sentimos, la duración de la música ha superado el límite máximo de 10 MB.",
                  id
                );
              const {
                image,
              ...
              await tobz
                .sendFileFromUrl(from, mp3, `${title}.mp3`, "", id)
                .catch(() => tobz.reply(from, mess.error.Yt4, id));
              await limitAdd(serial);
            }
          } catch (err) {
        ...
        case prefix + "ytmp3":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Enviar comando *#ytmp3 [Link Yt]*, por ejemplo, envíe el comando *#readme*`,
              id
            );
          let isLinks = args[1].match(
            /(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/
          );
          if (!isLinks) return tobz.reply(from, mess.error.Iv, id);
          try {
            tobz.reply(from, mess.wait, id);
            const vhtearyt3 = await fetch(
              `https://api.vhtear.com/ytdl?link=${args[1]}&apikey=${vhtearkey}`
            );
            if (!vhtearyt3.ok)
              throw new Error(`Error ytmp3 3 : ${vhtearyt3.statusText}`);
            ...
            if (vhtearyt33.status == false) {
              ...
              const {
                ...
                status,
                imgUrl,
              } = await vhtearyt33.result; // Ni Juga Sama Keatasin Aja Wkwk
              if (Number(vhtearyt33.result.size.split(" MB")[0]) >= 10.0)
                return tobz.sendFileFromUrl(
                  from,
                  vhtearyt33.result.imgUrl,
                  `thumb.jpg`,
                  `*「 YOUTUBE MP3 」*\n\n• *Titulo* : ${vhtearyt33.result.title}\n• *tamaño del archivo* : ${vhtearyt33.result.size}\n\n_Lo sentimos, la duración del audio supera los 10 MB. Descargue el audio a través del enlace a continuación_.\n${vhtearyt33.result.UrlMp3}`,
                  id
                );
              ...
              await tobz
                .sendFileFromUrl(from, UrlMp3, `${title}.mp3`, "", id)
                .catch(() => tobz.reply(from, mess.error.Yt4, id));
              await limitAdd(serial);
            }
          } catch (err) {
        ...
        case prefix + "google":
          ...
          if (googleQuery == undefined || googleQuery == " ")
            return tobz.reply(
              from,
              `*Resultado de búsqueda : ${googleQuery}* extraviado`,
              id
            );
          google({
            query: googleQuery,
          ...
          if (args.length >= 2) {
            ...
            translatte(texta, {
              ...
              .catch((err) => {
                tobz.sendText(
                  from,
                  `[ERROR] Falta texto o código de idioma ${codelang} no apoyo\n~> *${prefix}bahasa* para ver una lista de códigos de idioma`
                );
              });
          }
          break;
        case prefix + "nhentai": // SEARCH NHENTAI
          ...
          tobz.reply(
            from,
            "COMANDO PREMIUM, HABLE CON EL CREADOR DEL BOT : wa.me/18652809983",
            id
          );
          break;
        ...
        case prefix + "xnxx":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#xnxx [linkXnxx]*, por ejemplo, envíe el comando *#readme*"
            );
          if (!args[1].match(isUrl) && !args[1].includes("xnxx.com"))
            return tobz.reply(from, mess.error.Iv, id);
          try {
            tobz.reply(from, mess.wait, id);
            const resq = await axios.get(
              "https://mhankbarbars.herokuapp.com/api/xnxx?url=" +
                args[1] +
                "&apiKey=" +
                barbarkey
            );
            ...
            if (resp.error) {
              ...
              if (Number(resp.result.size.split(" MB")[0]) > 20.0)
                return tobz.reply(
                  from,
                  "Lo sentimos, la duración del video ha excedido el límite máximo de 20 minutos.",
                  id
                );
              tobz.sendFileFromUrl(
                from,
                resp.result.thumb,
                "thumb.jpg",
                `➸ *Titulo* : ${resp.result.judul}\n➸ *Descripcion* : ${resp.result.desc}\n➸ *Tamaño del archivo* : ${resp.result.size}\n\nEspere un momento a que el proceso de envío del archivo tarde unos minutos.`,
                id
              );
              await tobz.sendFileFromUrl(
                from,
                resp.result.vid,
                `${resp.result.title}.mp4`,
                "",
                id
              );
            }
            ...
            await tobz.sendFileFromUrl(
              ...
              errorurl2,
              "[error.png]",
              "💔️ Lo siento, no se encontró ningún video"
            );
        ...
        case prefix + "pareja":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#pareja [tú|pareja]*\nEjemplo:*#pareja Enrique|Paula*",
              id
            );
          arg = body.trim().split("|");
          if (args.length >= 2) {
            ...
            const rjh2 = `*¡Resultados de la observación! ${kamu} y ${pacar}\n\n➸ Amor : ${rpmn}\n➸ Conexion : ${rpmn2}\n➸ Semejanza : ${rpmn3}\n➸ Pasiones : ${rpmn4}\n➸ Gustos : ${rpmn5}\n➸ Posiblidades De Tener Sexo ${rpmn6}`;
            tobz.reply(from, rjh2, id);
            limitAdd(serial);
          } else {
            await tobz.reply(from, "Formato erroneo!", id);
          }
          break;
        case prefix + "artinama":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#artinama [query]*\nContoh : *#artinama Tobz*",
              id
            );
          try {
            const resp = await axios.get(
              "https://api.vhtear.com/artinama?nama=" +
                body.slice(9) +
                "&apikey=" +
                vhtearkey
            );
        ...
        case prefix + "zodiak":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#zodiak [zodiak kamu]*\nContoh : *#zodiak scorpio*",
              id
            );
          try {
            const resp = await axios.get(
              "https://api.vhtear.com/zodiak?query=" +
                body.slice(8) +
                "&apikey=" +
                vhtearkey
            );
        ...
        case prefix + "tebakgambar":
          ...
          try {
            const resp = await axios.get(
              "https://api.vhtear.com/tebakgambar&apikey=" + vhtearkey
            );
            if (resp.data.error) return tobz.reply(from, resp.data.error, id);
            const jwban = `➸ Jawaban : ${resp.data.result.jawaban}`;
            tobz.sendFileFromUrl(
              from,
              resp.data.result.soalImg,
              "tebakgambar.jpg",
        ...
        case prefix + "heroml":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#heroml [nama hero]*\nContoh : *#heroml akai*",
              id
            );
          try {
            const resp = await axios.get(
              "https://api.vhtear.com/herodetail?query=" +
                body.slice(8) +
                "&apikey=" +
                vhtearkey
            );
        ...
        case prefix + "nomorhoki":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#nomorhoki [no hp kamu]*\nContoh : *#nomorhoki 0895384009405*",
              id
            );
          try {
            const resp = await axios.get(
              "https://api.vhtear.com/nomerhoki?no=" +
                body.slice(11) +
                "&apikey=" +
                vhtearkey
            );
            ...
            await tobz.sendFileFromUrl(
              ...
              errorurl2,
              "error.png",
              "💔️ Maaf, Nomor Hoki tidak ditemukan"
            );
        ...
        case prefix + "artimimpi":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#artimimpi [mimpi]*\nContoh : *#artimimpi ular*",
              id
            );
          try {
            const resp = await axios.get(
        ...
        case prefix + "wiki":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#wiki [ Query ]*\nContoh : *#wiki asu*`,
              id
            );
          const queryz_ = body.slice(6);
          const wiki = await axios.get(
            `https://api.vhtear.com/wikipedia?query=${queryz_}&apikey=${vhtearkey}`
          );
          if (wiki.data.error) {
            ...
            tobz.sendFileFromUrl(
              from,
              wiki.data.result.ImgResult,
              "",
              `*「 WIKI 」*\n\n➸ *Query* : ${queryz_}\n\n➸ *Result* : ${wiki.data.result.Info}`,
              id
            );
            await limitAdd(serial);
          }
        case prefix + "kbbi":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#kbbi [ Query ]*\nContoh : *#kbbi asu*`,
              id
            );
          const kbbl = body.slice(6);
          const kbbl2 = await axios.get(
            `https://api.vhtear.com/kbbi?query=${kbbl}&apikey=${vhtearkey}`
          );
          ...
          if (kbbl2.data.error) {
            ...
            tobz.sendText(
              from,
              `*「 KBBI 」*\n\n➸ *Query* : ${kbbl}\n\n➸ *Result* : ${kbbl2.data.result.hasil}`,
              id
            );
            await limitAdd(serial);
          }
        ...
        case prefix + "googleimage":
          ...
          if (argz.length >= 2) {
            ...
            if (!qwery)
              return await tobz.reply(
                from,
                `Enviar comando *#googleimage [| Consulta | Cantidad]*, ejemplo = #googleimage | loli | 3`,
                id
              );
            if (!jum)
              return await tobz.reply(
                from,
                `Número de dibujos necesarios, p. Ej. = #googleimage |loli|3`,
                id
              );
            if (jum >= 5)
              return await tobz.reply(
                from,
                "¡Demasiados números! Máximo 4",
                id
              );
            ...
            function logResults(error, results) {
              if (error) {
                tobz.reply(
                  from,
                  "Lo sentimos, la función está actualmente en error",
                  id
                );
              } else {
                ...
                item.forEach(async (res) => {
                  ...
                  tobz.sendImage(
                    ...
                    res.url,
                    null,
                    `➸ Link : ${yurl}\n➸ Tamaño de la imagen : ${res.height} x ${res.width}`
                  );
                  limitAdd(serial);
                });
              }
        ...
        case prefix + "sandwriting":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#sandwriting [Texto]* \nEjemplo *#sandwriting hola*",
              id
            );
...
*Text : ${swrt}*`;
            ...
            await tobz.sendFileFromUrl(
              ...
              errorurl2,
              "[error.png]",
              "💔️ Lo sentimos, usuario no encontrado"
            );
        ...
        case prefix + "tahta":
          ...
          if (!jreng)
            return tobz.reply(
              from,
              "Enviar comando *#tahta [texto]*  n  nEjemplo *#tahta hola*",
              id
            );
          if (jreng.length > 7) return tobz.reply(from, "Máximo 7 letras!", id);
          tobz.sendText(
            from,
            "_En progreso, por favor espere un momento!..._",
            id
          );
          await tobz.sendFileFromUrl(
            from,
            `https://api.vhtear.com/hartatahta?text=${jreng}&apikey=${vhtearkey}`,
            `${jreng}.jpg`,
            `Tesoro del trono ${jreng}`,
            id
          );
          await limitAdd(serial);
          break;
        case prefix + "resepmasakan":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#resepmasakan [opcional]* \nEjemplo *#resepmasakan hamburguesa*",
              id
            );
          arg = body.trim().split(" ");
          console.log(...arg[1]);
          ...
          try {
            const resp = await axios.get(
              "https://api.vhtear.com/resepmasakan?query=" +
                rmk +
                "&apikey=" +
                vhtearkey
            );
...
➸ *Camino:* ${cara}`;
            ...
            await tobz.sendFileFromUrl(
              ...
              errorurl2,
              "[error.png]",
              "💔️ Lo siento, no se encontró ninguna receta"
            );
        ...
        case prefix + "twtstalk":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#twtstalk @username* \nEjemplo *#twtstalk @miakhalifah*",
              id
            );
          arg = body.trim().split(" ");
          console.log(...arg[1]);
          ...
          console.log(twstalk);
          try {
            const twstalk2 = await axios.get(
              "https://mhankbarbars.herokuapp.com/api/twstalk?username=" +
                twstalk +
                "&apiKey=" +
                barbarkey
            );
            ...
            const twstalk3 = `*Usuario encontrado!*

➸ *Nombre:* ${name}
➸ *Nombre completo:* ${full_name}
➸ *Numero de seguidores:* ${followers_count}
➸ *Número de publicaciones:* ${status_count}`;
        ...
        case prefix + "igstalk":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Enviar comando *#igstalk @ username* \nEjemplo *#igstalk @xyarby*",
              id
            );
          arg = body.trim().split(" ");
          console.log(...arg[1]);
          ...
          console.log(istalk);
          try {
            const istalk2 = await axios.get(
              "https://tobz-api.herokuapp.com/api/stalk?username=" + istalk
            );
            const {
              Biodata,
              Jumlah_Followers,
            ...
            const istalk3 = `*Usuario encontrado!*

➸ *Username:* ${Username}
➸ *Nombre:* ${Name}
➸ *Bio:* ${Biodata}
➸ *Seguidos:* ${Jumlah_Following}
➸ *Seguidores:* ${Jumlah_Followers}
➸ *Numero de publicaciones:* ${Jumlah_Post}`;
        ...
        case prefix + "tiktokstalk":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#tiktokstalk @username*\nContoh *#tiktokstalk @duar_amjay*",
              id
            );
          arg = body.trim().split(" ");
          console.log(...arg[1]);
          ...
          console.log(tstalk);
          try {
            const tstalk2 = await axios.get(
              "https://api.vhtear.com/tiktokprofile?query=" +
                tstalk +
                "&apikey=" +
                vhtearkey
            );
...
➸ *Link:* ${url_account}`;
        ...
        case prefix + "smulestalk":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#smulestalk [@username]*\nContoh : *#smulestalk loli*",
              id
            );
          arg = body.trim().split(" ");
          console.log(...arg[1]);
          ...
          console.log(sstalk);
          try {
            const sstalk2 = await axios.get(
              "https://api.vhtear.com/smuleprofile?query=" +
                sstalk +
                "&apikey=" +
                vhtearkey
            );
...
➸ *Total Rekaman:* ${recording}`;
        ...
        case prefix + "":
          ...
          if (!isSimi)
            return tobz.reply(
              from,
              "command/Perintah Simi belum di aktifkan di group ini!",
              id
            );
          if (args.length === 1)
            return tobz.reply(
          ...
          const sigo = await axios.get(
            `http://simsumi.herokuapp.com/api?text=${que}&lang=id`
          );
        ...
        case prefix + "instagram":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#ig [ Link Instagram ]* untuk contoh silahkan kirim perintah *#readme*`
            );
          if (!args[1].match(isUrl) && !args[1].includes("instagram.com"))
            return tobz.reply(
              from,
              `Maaf, link yang kamu kirim tidak valid. [Invalid Link]`,
              id
            );
        ...
        case prefix + "fb":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#fb [ Link Fb ]*\nContoh : *#fb https://www.facebook.com/24609282673/posts/10158628585367674/*`,
              id
            );
          tobz.reply(from, mess.wait, id);
          facebook(args[1])
        ...
        case prefix + "waktu":
          ...
          await tobz.sendText(
            from,
            `Waktu Indonesia Barat: *${moment()
              .utcOffset("+0700")
              .format(
                "HH:mm"
              )}* WIB \nWaktu Indonesia Tengah: *${moment()
              .utcOffset("+0800")
              .format(
                "HH:mm"
              )}* WITA \nWaktu Indonesia Timur: *${moment()
              .utcOffset("+0900")
              .format("HH:mm")}* WIT`
          );
          await limitAdd(serial);
        ...
        case prefix + "tiktok":
          ...
          tiktok(args[1])
            .then(async (res) => {
              ...
              await tobz
                .sendFileFromUrl(from, video, `${title}.mp4`, "", id)
                .catch(() => tobz.reply(from, mess.error.Yt4, id));
              await limitAdd(serial);
            })
            .catch((err) => {
        ...
        case prefix + "smule":
          ...
          smule(args[1])
            .then(async (res) => {
              ...
              await tobz
                .sendFileFromUrl(from, url, `${title}.mp3`, "", id)
                .catch(() => tobz.reply(from, mess.error.Yt4, id));
              await limitAdd(serial);
            })
            .catch((err) => {
        ...
        case prefix + "starmaker":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#starmaker [linkStarmaker]* untuk contoh silahkan kirim perintah *#readme*"
            );
          tobz.reply(from, mess.wait, id);
          starmaker(args[1])
            .then(async (res) => {
        ...
        case prefix + "twitter":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#twitter [ Link Twitter ]* untuk contoh silahkan kirim perintah *#readme*`
            );
          tobz.reply(from, mess.wait, id);
          twitter(args[1])
            .then(async (res) => {
              ...
              await tobz
                .sendFileFromUrl(from, urlVideo, `twit.mp3`, ttwitter, id)
                .catch(() => tobz.reply(from, mess.error.Yt4, id));
              await limitAdd(serial);
            })
            .catch((err) => {
        ...
        case prefix + "maps":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#maps [optional]*, Contoh : *#maps Jakarta*"
            );
          arg = body.trim().split(" ");
          console.log(...arg[1]);
          var slicedArgs = Array.prototype.slice.call(arg, 1);
        ...
        case prefix + "joox":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#joox [ Optional ]*\nContoh : *#joox Alan Walker*`,
              id
            );
          tobz.reply(from, mess.wait, id);
          joox(args[1])
            .then(async (res) => {
              ...
              tobz
                .sendFileFromUrl(from, linkMp3, `${judul}.${ext}`, "", id)
                .catch(() => tobz.reply(from, mess.error.Yt4, id));
              await limitAdd(serial);
            })
            .catch((err) => {
        ...
        case prefix + "checkip":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#checkip [ipaddress]*\nContoh : *#checkip 182.0.144.145*",
              id
            );
          tobz.reply(from, mess.wait, id);
          arg = body.trim().split(" ");
          ...
          console.log(cekip);
          try {
            const cekip2 = await axios.get(
              "https://mnazria.herokuapp.com/api/check?ip=" + cekip
            );
            const {
              city,
              continent_name,
...
➸ *Wilayah:* +${region_name}`;
        ...
        case prefix + "nulis":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#nulis [teks]*, contoh *#nulis aku bukan boneka*",
              id
            );
          const ngettik = body.slice(7);
          const ngetikk = await axios.get(
            "https://mhankbarbars.herokuapp.com/nulis?text=" +
              ngettik +
              "&apiKey=" +
              barbarkey
          );
          if (ngetikk.data.error)
            return tobz.reply(from, ngetikk.data.error, id);
        ...
        case prefix + "ptl":
          ...
          tobz.sendFileFromUrl(
            ...
            pep,
            "pptl.jpg",
            "Follow ig : https://www.instagram.com/ptl_repost untuk mendapatkan penyegar timeline lebih banyak",
            message.id
          );
          await limitAdd(serial);
          break;
        ...
        case prefix + "quotes":
          ...
          const quotez2 = await axios.get(
            "https://tobz-api.herokuapp.com/api/randomquotes"
          );
          tobz.reply(
            from,
            `➸ *Quotes* : ${quotez2.data.quotes}\n➸ *Author* : ${quotez2.data.author}`,
            id
          );
          await limitAdd(serial);
          break;
        ...
        case prefix + "chord":
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              "Kirim perintah *#chord [query]*, contoh *#chord aku bukan boneka*",
              id
            );
          const query__ = body.slice(7);
          const chord = await axios.get(
            "https://tobz-api.herokuapp.com/api/chord?q=" + query__
          );
        ...
        case prefix + "listdaerah":
          ...
          const listDaerah = await axios.get(
            "https://tobz-api.herokuapp.com/api/daerah"
          );
        ...
        case prefix + "cerpen": // ARUGAZ
          ...
          const cerpen = await get
            .get("https://arugaz.herokuapp.com/api/cerpen")
            .json();
        ...
        case prefix + "puisi": // ARUGAZ
          ...
          const puisi = await get
            .get("https://arugaz.herokuapp.com/api/puisi1")
            .json();
        ...
        case prefix + "puisi2": // ARUGAZ
          ...
          const puisi2 = await get
            .get("https://arugaz.herokuapp.com/api/puisi2")
            .json();
        ...
        case prefix + "puisi3": // ARUGAZ
          ...
          const puisi3 = await get
            .get("https://arugaz.herokuapp.com/api/puisi3")
            .json();
        ...
        case prefix + "setprefix":
          if (!isOwner)
            return tobz.reply(
              from,
              `Perintah ini hanya bisa di gunakan oleh Owner Elaina!`,
              id
            );
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *${prefix}prefix [ NEW PREFIX ]*`,
              id
            );
          ...
          tobz.sendText(
            from,
            `Berhasil Mengganti Prefix Ke *「* ${prefa} *」*`
          );
          break;
        case prefix + "addbadword":
          ...
          if (!args.length >= 1)
            return tobz.reply(
              from,
              `Masukkan kata kasar yang akan di blacklist `,
              id
            );
          ...
          if (cek) {
            ...
            tobz.reply(
              from,
              `Success Menambahkan Blacklist Badword\nTotal Data Badword Sekarang : *${
                dbbadword.length - 1
              }*`,
              id
            );
        ...
        case prefix + "bc": // KASIH CREDIT DONG KALO COPAS
          if (!isOwner)
            return tobz.reply(
              from,
              `Perintah ini hanya untuk Owner Elaina`,
              id
            );
          bctxt = body.slice(4);
          ...
          if (quotedMsg && quotedMsg.type == "image") {
            ...
            for (let grupnya of semuagrup) {
              ...
              if (!cekgrup.isReadOnly)
                tobz.sendImage(grupnya, imageBase64, "gambar.jpeg", txtbc);
            }
        ...
        case prefix + "okickall":
          ...
          if (!isOwner)
            return tobz.reply(
              from,
              "Perintah ini hanya untuk Admin Elaina",
              id
            );
          if (!isBotGroupAdmins)
            return tobz.reply(
        ...
        case prefix + "okick":
          ...
          if (mentionedJidList.length === 0)
            return tobz.reply(
              from,
              "Untuk menggunakan Perintah ini, kirim perintah *#okick* @tagmember",
              id
            );
          await tobz.sendText(
            from,
            `Perintah Owner diterima, mengeluarkan:\n${mentionedJidList.join(
              "\n"
            )}`
          );
          for (let i = 0; i < mentionedJidList.length; i++) {
            if ((adminNumber, ownerNumber).includes(mentionedJidList[i]))
              return tobz.reply(from, mess.error.Sp, id);
        ...
        case prefix + "kick":
          ...
          for (let i = 0; i < mentionedJidList.length; i++) {
            if ((adminNumber, groupAdmins).includes(mentionedJidList[i]))
              return tobz.reply(from, mess.error.Sp, id);
            await tobz.removeParticipant(groupId, mentionedJidList[i]);
          }
          break;
        case prefix + "oleave":
          ...
          await tobz
            .sendText(from, "ELAINA DIPERINTAHKAN KELUAR OLEH OWNER!!")
            .then(() => tobz.leaveGroup(groupId));
          break;
        case prefix + "leave":
          if (!isGroupMsg)
            return tobz.reply(
        ...
        case prefix + "opromote":
          ...
          await tobz.sendTextWithMentions(
            from,
            `Perintah Owner diterima, menambahkan @${mentionedJidList[0]} sebagai admin.`
          );
          break;
        case prefix + "promote":
          if (!isGroupMsg)
        ...
        case prefix + "odemote":
          ...
          await tobz.sendTextWithMentions(
            from,
            `Perintah Owner diterima, menghapus jabatan @${mentionedJidList[0]}.`
          );
          break;
        case prefix + "demote":
          if (!isGroupMsg)
        ...
        case prefix + "join":
          if (args.length === 1)
            return tobz.reply(
              from,
              "Hanya Owner yang bisa memasukan Bot ke dalam Grup!",
              id
            );
          if (!isOwner)
            return tobz.reply(
          ...
          if (tGr.length > 256)
            return tobz.reply(from, "Maaf jumlah group sudah maksimal!", id);
          if (check.size < minMem)
            return tobz.reply(
              from,
              "Member group tidak melebihi 5, bot tidak bisa masuk",
              id
            );
          if (check.status === 200) {
            await tobz
              .joinGroupViaLink(link)
              .then(() => tobz.reply(from, "Bot akan segera masuk!"));
          } else {
        ...
        case prefix + "sider":
          ...
          if (!quotedMsg)
            return tobz.reply(from, `Tolong Reply Pesan Elaina`, id);
          if (!quotedMsgObj.fromMe)
            return tobz.reply(from, `Tolong Reply Pesan Elaina`, id);
          try {
            ...
            tobz.reply(
              from,
              `Maaf, Belum Ada Yang Membaca Pesan Elaina atau Mereka Menonaktifkan Read Receipts`,
              id
            );
          }
          break;
        ...
        case prefix + "resetlinkgroup":
          ...
          if (isGroupMsg) {
            ...
            tobz.sendTextWithMentions(
              from,
              `Link group telah direset oleh admin @${sender.id.replace(
                "@c.us",
                ""
              )}`
            );
        ...
        case prefix + "limit":
          ...
          if (found === false) {
            ...
            tobz.reply(
              from,
              `Sisa limit request anda tersisa : *${limitCount}*\n\n_Note : Limit akan direset setiap jam 21:00!_`,
              id
            );
          }
          break;
        case prefix + "eval":
          ...
          if (!q)
            return tobz.reply(from, "Harap masukkan code JavaScript!", id);
          try {
        ...
        case prefix + "setprofilepic":
          ...
          if (isMedia) {
            ...
            tobz.sendTextWithMentions(
              `Makasih @${sender.id.replace("@c.us", "")} Foto Profilenya 😘`
            );
          } else if (quotedMsg && quotedMsg.type == "image") {
            const mediaData = await decryptMedia(quotedMsg);
            const imageBase64 = `data:${
            ...
            tobz.reply(
              from,
              `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan #setprofilepic`,
              id
            );
          }
          break;
        ...
        case prefix + "ban":
          ...
          for (let i = 0; i < mentionedJidList.length; i++) {
            if (adminNumber.includes(mentionedJidList[i]))
              return tobz.reply(
                from,
                `Maaf ${pushname}, Kamu tidak bisa banned Admin Elaina!`,
                id
              );
        ...
        case prefix + "setgroupicon":
          ...
          if (isMedia) {
            ...
            tobz.sendTextWithMentions(
              from,
              `Profile group telah diubah oleh admin @${sender.id.replace(
                "@c.us",
                ""
              )}`
            );
            ...
            tobz.reply(
              from,
              `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan #setgroupicon`,
              id
            );
          }
          break;
        ...
        case prefix + "elainagroup":
          tobz.reply(
            from,
            `Link Group Elaina : https://chat.whatsapp.com/By906EiJBGBCZGURDadOat\nJangan Lupa Join Ya Kak ${pushname}`,
            id
          );
          break;
        ...
        case prefix + "wame":
          await tobz.reply(
            from,
            `*Neh Mhank Link Nomor Wa Lu ${pushname}*\n\n*wa.me/${sender.id.replace(
              /[@c.us]/g,
              ""
            )}*\n\n*Atau*\n\n*api.whatsapp.com/send?phone=${sender.id.replace(
              /[@c.us]/g,
              ""
            )}*`
          );
        ...
        default:
          ...
          if (command.startsWith("#")) {
            tobz.reply(
              from,
              `Maaf ${pushname}, Command *${args[0]}* Tidak Terdaftar Di Dalam *#menu*!`,
              id
            );
          }
          await tobz.sendSeen(from);
